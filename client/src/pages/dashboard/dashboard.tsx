...
export const Dashboard = () => {
  ...
  const fetchUsers = async () => {
    ...
    const calculateStats=(userList: IUser[])=>{
        ...
        userList.forEach((user: IUser ) => {
            if(user?.role==="ADMIN")  { AdminCount++; }
              ...
              if(user.lastLogin)
              {
                ...
                if(diffInDays<=30)  {  ActiveUser++; }
              }
            if(user.createdAt)
            {
              const date=new Date(user.createdAt);
              const MonthYear=`${date.toLocaleString("default",{month:"short"})} ${date.getFullYear()}`
              if(!userCounts[MonthYear])  { userCounts[MonthYear]=0; }
  ...
  useEffect(() => {
    ...
    { name: "Moderator", value: totalUsers - totalAdmin - totalClient },
  ];
  ...
  return (
    ...
    <StatCard title="Active Users" value={totalactiveUsers} icon={TaskAltIcon} color="green" />
      ...
      <Stack direction="row" spacing={6} alignItems="center" justifyItems="sapce-around">
          {/* <PieChart data={Object.values(userCounts)} categories={Object.keys(userCounts)}></PieChart> */}
          <BarChart userCounts={(userCounts)}></BarChart>
          <PieChart roleData={(roleData)}></PieChart>
      </Stack>
